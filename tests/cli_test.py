...
def test_calling_cli_creates_expected_file(
    ...
    cli_function(
        command_line_args=predict_args(
    ...
    with open(output_file, encoding="utf-8") as prediction_file:
        assert prediction_file.readlines() == [
            "shot\n",
            "shot L0\n",
            "shot L2\n",
            "shot L1\n",
...
def test_patching_cli_argv_creates_expected_file(
    tmp_path: Path, data_dir: Path, input_format: str
):
    output_file = tmp_path / "three_errors_predictions.dets"
    args = predict_args(
        data_dir / "three_errors.dem",
...
def test_load_surface_code_b8_cli(tmp_path: Path, data_dir: Path):
    ...
    out_fn = (
        tmp_path
        / "surface_code_rotated_memory_x_13_0.01_1000_shots_temp_predictions.b8"
    )

    pymatching._cpp_pymatching.main(
        command_line_args=[
            "predict",
            "--dem",
            str(dem_path),
