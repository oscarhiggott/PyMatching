...
def test_matching_to_networkx():
    ...
    assert list(g.edges(data=True)) == [
        (0, 1, {"weight": 1.0, "error_probability": -1, "fault_ids": set()})
    ]
...
def test_matching_to_rustworkx():
    ...
    m.add_edge(0, 1)
    g = m.to_rustworkx()
    assert list(g.weighted_edge_list()) == [
        (0, 1, {"weight": 1.0, "error_probability": -1, "fault_ids": set()})
    ]
...
def test_negative_weight_edge_returned():
    ...
    expected = [
        (0, 1, {"fault_ids": set(), "weight": 0.5, "error_probability": 0.3}),
...
def test_self_loop_to_networkx():
    ...
    g = m.to_networkx()
    assert list(g.edges(data=True)) == [
        (0, 0, {"fault_ids": set(), "weight": 3.0, "error_probability": -1.0}),
