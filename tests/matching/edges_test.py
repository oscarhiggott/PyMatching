...
def test_add_edge():
    ...
    expected = [
        (0, 1, {"fault_ids": set(), "weight": 0.123, "error_probability": 0.6}),
...
def test_add_edge_merge_strategy():
    ...
    assert m.edges() == [
        (0, 10, {"fault_ids": {0}, "weight": 1.2, "error_probability": 0.3})
    ]
    m.add_edge(
        0,
        10,
    ...
    assert m.edges() == [
        (0, 10, {"fault_ids": {1}, "weight": 1.0, "error_probability": 0.35})
    ]
    with pytest.raises(ValueError):
        m.add_edge(
            0,
    ...
    assert es == [
        (0, 10, {"fault_ids": {1}, "weight": 0.958128, "error_probability": 0.41})
    ]
    m = Matching()
    ...
    assert m.edges() == [
        (1, 10, {"fault_ids": {0}, "weight": 2, "error_probability": 0.3})
    ]
    m.add_edge(
        1, 10, fault_ids={2}, weight=5, error_probability=0.1, merge_strategy="replace"
    )
    assert m.edges() == [
        (1, 10, {"fault_ids": {2}, "weight": 5, "error_probability": 0.1})
    ]
...
def test_add_boundary_edge():
    ...
    assert m.edges() == [
        (0, None, {"fault_ids": {0}, "weight": 1.2, "error_probability": 0.3})
    ]
    m.add_boundary_edge(
        0,
        fault_ids={1},
    ...
    assert m.edges() == [
        (0, None, {"fault_ids": {1}, "weight": 1.0, "error_probability": 0.35})
    ]
    with pytest.raises(ValueError):
        m.add_boundary_edge(
            0,
    ...
    es[0][2]["weight"] = round(es[0][2]["weight"], 6)
    assert es == [
        (0, None, {"fault_ids": {1}, "weight": 0.958128, "error_probability": 0.41})
    ]
    m = Matching()
    ...
    assert m.edges() == [
        (1, None, {"fault_ids": {0}, "weight": 2, "error_probability": 0.3})
    ]
    m.add_boundary_edge(
        1, fault_ids={2}, weight=5, error_probability=0.1, merge_strategy="replace"
    )
    assert m.edges() == [
        (1, None, {"fault_ids": {2}, "weight": 5, "error_probability": 0.1})
    ]
...
def test_get_edge_data():
    ...
    assert m.get_edge_data(0, 1) == {
        "fault_ids": {0, 1, 2},
        "weight": 2.5,
        "error_probability": 0.1,
    }
    m.add_boundary_edge(5, {5, 6, 7}, 5.0, 0.34)
    assert m.get_boundary_edge_data(5) == {
        "fault_ids": {5, 6, 7},
        "weight": 5.0,
        "error_probability": 0.34,
    }
