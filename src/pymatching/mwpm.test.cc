...
struct MwpmAltTreeTestData {
    ...
    pm::AltTreeEdge t(
        std::vector<pm::AltTreeEdge> children, size_t inner_region_id, size_t outer_region_id, bool root = false) const;
};
...
pm::MwpmEvent rhr(std::vector<pm::DetectorNode>& ns, size_t i, size_t j) {
    return pm::RegionHitRegionEventData{ns[i].region_that_arrived, ns[j].region_that_arrived, {&ns[i], &ns[j], 0}};
}
...
pm::AltTreeEdge MwpmAltTreeTestData::t(
    std::vector<pm::AltTreeEdge> children, size_t inner_region_id, size_t outer_region_id, bool root) const {
    pm::AltTreeNode* node;
...
TEST(Mwpm, BlossomCreatedThenShattered) {
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(10), 1024));
    ...
    ASSERT_EQ(
        e5,
        pm::MwpmEvent(pm::RegionHitRegionEventData(
    ...
    ASSERT_EQ(mwpm.flooder.queue.cur_time, 25);
    // Region 6 collides with matched blossom
    mwpm.process_event(mwpm.flooder.next_event());
    ASSERT_EQ(mwpm.flooder.queue.cur_time, 83);
    ASSERT_EQ(
    ...
    ASSERT_EQ(mwpm.flooder.queue.cur_time, 91);
    ...
    ASSERT_EQ(ns[5].region_that_arrived->match, pm::Match(nullptr, {&ns[5], nullptr, 8}));
}

TEST(Mwpm, BlossomShatterDrivenWithoutFlooder) {
    size_t n = 10;
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(n + 3), 2048));
    auto& ns = mwpm.flooder.graph.nodes;
    ...
    mwpm.process_event(pm::RegionHitRegionEventData{ns[n + 1].region_that_arrived, blossom, {&ns[n + 1], &ns[5], 0}});
    mwpm.process_event(pm::RegionHitRegionEventData{ns[n + 2].region_that_arrived, blossom, {&ns[n + 2], &ns[10], 0}});
    ASSERT_EQ(blossom->blossom_children.size(), 11);
    mwpm.process_event(pm::BlossomShatterEventData{blossom, ns[10].region_that_arrived, ns[5].region_that_arrived});
...
TEST(Mwpm, BranchingTreeFormsBlossomThenHitsBoundaryMatch) {
    size_t n = 14;
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(n), 2048));
    auto& ns = mwpm.flooder.graph.nodes;
    ...
    mwpm.process_event(pm::RegionHitBoundaryEventData{ns[13].region_that_arrived, {&ns[13], nullptr, 0}});
    // Blossom matches to region 13
    mwpm.process_event(
        pm::MwpmEvent(pm::RegionHitRegionEventData(blossom, ns[13].region_that_arrived, {&ns[4], &ns[13], 1})));
    ASSERT_TRUE(regions_matched(ns, 5, 9));
    ASSERT_TRUE(regions_matched(ns, 8, 12));
    ASSERT_EQ(blossom->match, pm::Match(ns[13].region_that_arrived, {&ns[4], &ns[13], 1}));
    ASSERT_EQ(ns[13].region_that_arrived->match, pm::Match(blossom, {&ns[13], &ns[4], 1}));
}
...
TEST(Mwpm, BoundaryMatchHitsTree) {
    ...
    mwpm.process_event(pm::RegionHitBoundaryEventData{ns[5].region_that_arrived, {&ns[5], nullptr, 1}});
    // Tree matches to 5
...
TEST(Mwpm, ShatterBlossomAndExtractMatchesForPair) {
    size_t num_nodes = 20;
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(num_nodes), 2048));
    auto &g = mwpm.flooder.graph;
...
TEST(Mwpm, ShatterBlossomAndExtractMatches) {
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(12), 2048));
    ...
    mwpm.process_event(pm::RegionHitRegionEventData{blossom2, blossom3, {&ns[5], &ns[8], 1 << 12}});
    mwpm.process_event(pm::RegionHitRegionEventData{ns[9].region_that_arrived, blossom2, {&ns[9], &ns[5], 1 << 13}});
    mwpm.process_event(pm::RegionHitRegionEventData{ns[9].region_that_arrived, blossom3, {&ns[9], &ns[8], 1 << 14}});
    ...
    mwpm.process_event(pm::RegionHitRegionEventData{blossom1, blossom4, {&ns[0], &ns[5], 1 << 15}});
...
TEST(Mwpm, ShatterAndMatchBlossomMatchedToBoundary) {
    auto mwpm = pm::Mwpm(pm::GraphFlooder(pm::MatchingGraph(5), 2048));
    auto& ns = mwpm.flooder.graph.nodes;
    pm::time_int w = 0;
    for (auto& n : ns) {
        w += 1;
    ...
    mwpm.process_event(pm::RegionHitRegionEventData{blossom1, ns[3].region_that_arrived, {&ns[0], &ns[3], 1 << 10}});
    mwpm.process_event(pm::RegionHitRegionEventData{blossom1, ns[4].region_that_arrived, {&ns[0], &ns[4], 1 << 20}});
    auto blossom2 = ns[3].region_that_arrived->blossom_parent;
    ASSERT_EQ(blossom2->blossom_children.size(), 3);
    blossom2->radius = blossom2->radius + 30;
    mwpm.process_event(pm::RegionHitBoundaryEventData{blossom2, {&ns[0], nullptr, 1 << 30}});
